...
void create_extcall(extcall_t arr, void(*func)(void), void* emu_stack, void* opaque)
{
    ...
    arr[8] = opaque;
    // store arguments
    arr[9] = __builtin_gadget_addr("mov rax, rsi");
    arr[10] = __builtin_gadget_addr("pop rsi");
    arr[11] = st - 5;
    arr[12] = __builtin_gadget_addr("mov [rsi], rax ; mov al, 1");
    arr[13] = __builtin_gadget_addr("mov rax, rdx");
    arr[14] = __builtin_gadget_addr("pop rsi");
    arr[15] = st - 4;
    arr[16] = __builtin_gadget_addr("mov [rsi], rax ; mov al, 1");
    arr[17] = __builtin_gadget_addr("mov rax, rcx");
    arr[18] = __builtin_gadget_addr("pop rsi");
    arr[19] = st - 3;
    arr[20] = __builtin_gadget_addr("mov [rsi], rax ; mov al, 1");
    arr[21] = __builtin_gadget_addr("pop rcx");
    arr[22] = st - 6;
    arr[23] = __builtin_gadget_addr("mov [rcx], rdi");
    arr[24] = __builtin_gadget_addr("pop rdi");
    arr[25] = st - 2;
    arr[26] = __builtin_gadget_addr("mov [rdi], r8");
    arr[27] = __builtin_gadget_addr("pop rdi");
    arr[28] = st - 1;
    arr[29] = __builtin_gadget_addr("mov [rdi], r9");
